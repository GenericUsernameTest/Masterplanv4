...
function initializeNavDropdown() {
  ...
  const apiBase = "http://127.0.0.1:4000"; // keep consistent with main.js export function
        ...
        // --- Inline block placement using proper CAD transform ---
        try {
          const response = aiResponse;
          if (response && response.placementCoordinates && response.blockType) {
            const blockName = response.blockType;
            const [lng, lat] = response.placementCoordinates;
            const rotation = response.rotationAngle || 0;
            const inset = response.insetOffsetMeters || 0;
            const edgeBearing = response.edgeBearing || 0;
            
            console.log(`[AI Placement] blockType=${blockName}, coords=[${lng},${lat}], rotation=${rotation}, edgeBearing=${edgeBearing}, inset=${inset}`);

            const sourceId = `${blockName}-ai-placed`;
            const layerId = `${blockName}-ai-placed`;

            // Clean old layers
            if (map.getLayer(layerId)) map.removeLayer(layerId);
            if (map.getSource(sourceId)) map.removeSource(sourceId);

            // Get block data (prefer preloaded)
            let blockData = window.loadedBlocks?.[blockName];
            if (!blockData) {
              console.warn(`[AI Placement] ${blockName} not preloaded, fetching...`);
              fetch(`components/blocks/${blockName}.geojson`)
                .then(r => r.json())
                .then(data => {
                  if (window.loadedBlocks) window.loadedBlocks[blockName] = data;
                  placeTransformedBlock(data);
                })
                .catch(err => console.error('[AI Placement] Fetch failed:', err));
              return;
            }

            placeTransformedBlock(blockData);

            function placeTransformedBlock(rawBlock) {
              console.log('[AI Placement] Raw feature count=', rawBlock?.features?.length || 0);
              if (rawBlock?.features?.[0]) {
                console.log('[AI Placement] First coord sample=', JSON.stringify(rawBlock.features[0].geometry.coordinates[0]));
              }

              // Use the proper CAD-to-geographic transform
              if (typeof window.transformBlockToGeo !== 'function') {
                console.error('[AI Placement] transformBlockToGeo not available!');
                return;
              }

              const transformed = window.transformBlockToGeo(rawBlock, lng, lat, rotation);
              if (!transformed) {
                console.error('[AI Placement] Transform failed');
                return;
              }

              // NOTE: placementCoordinates already include inset offset from AI script
              // No need to apply additional translation here
              const final = transformed;

              if (typeof turf !== 'undefined') {
                console.log('[AI Placement] Transformed bbox=', turf.bbox(final));
              }

              const hasPolygons = final.features.some(f => ['Polygon','MultiPolygon'].includes(f.geometry.type));
              
              map.addSource(sourceId, { type: 'geojson', data: final });
              map.addLayer({
                id: layerId,
                type: hasPolygons ? 'fill' : 'line',
                source: sourceId,
                paint: hasPolygons 
                  ? { 'fill-color': '#ff4444', 'fill-opacity': 0.4 }
                  : { 'line-color': '#ff4444', 'line-width': 4, 'line-opacity': 0.9 }
              });
              
              console.log(`✅ [AI Placement] Layer added: ${layerId} (${hasPolygons ? 'fill' : 'line'})`);
            }
          } else {
            console.warn('[AI Placement] Missing placementCoordinates or blockType');
          }
        } catch (placementErr) {
          console.error('[AI Placement] Exception:', placementErr);
        }
        // --- End inline block placement ---

        // Debug: fetch site status
        try {
          const statusRes = await fetch(`${apiBase}/site-status/${siteId}`);
          if (statusRes.ok) {
            const statusJson = await statusRes.json();
            console.log('🩺 Site status after AI run:', statusJson);
          }
        } catch (statusErr) {
          console.warn('⚠️ Could not retrieve site status:', statusErr);
        }

        // === Use AI recommendations to highlight relevant boundaries ===
        (function highlightRecommendedEdges() {
          if (!window.siteBoundary || !window.map) {
            console.warn("Missing siteBoundary or map for edge highlighting");
            return;
          }

          const coords = window.siteBoundary.geometry?.coordinates?.[0];
          if (!Array.isArray(coords) || coords.length < 2) {
            console.warn("Invalid siteBoundary coordinates");
            return;
          }
          ...
          // Process AI recommendations to highlight edges
          const recommendations = aiResponse.recommendations || [];
          const highlightEdges = recommendations.filter(rec => 
            rec.action === "highlight_edge" || rec.action === "create_access"
          );

          // If no specific recommendations, highlight the southernmost edge as fallback
          if (highlightEdges.length === 0) {
            console.log("No AI recommendations found, highlighting southernmost edge");
            let minLat = Infinity, minIdx = -1;
            for (let i = 0; i < cleanCoords.length; i++) {
              const a = cleanCoords[i];
              const b = cleanCoords[(i + 1) % cleanCoords.length];
            ...
            if (minIdx !== -1) {
              highlightEdges.push({
                ...
                edgeIndex: minIdx,
                color: "red",
                reasoning: "Fallback: southernmost edge"
              });
            }
          }

          // Highlight each recommended edge
          highlightEdges.forEach((rec, index) => {
            const edgeIndex = rec.edgeIndex !== undefined ? rec.edgeIndex : 
                            rec.edgeId ? parseInt(rec.edgeId.replace(/\D/g, '')) || 0 : 0;
            const color = rec.color || "#ff2222";
            
            if (edgeIndex >= 0 && edgeIndex < cleanCoords.length) {
              const edgeCoords = [
                cleanCoords[edgeIndex],
                cleanCoords[(edgeIndex + 1) % cleanCoords.length]
              ];

              const geojson = {
                type: "Feature",
                geometry: { type: "LineString", coordinates: edgeCoords },
                properties: { 
                  name: `ai-recommended-edge-${index}`,
                  action: rec.action,
                  reasoning: rec.reasoning || "AI recommendation"
                }
              };

              const sourceId = `ai-edge-highlight-${index}`;
              const layerId = `ai-edge-highlight-layer-${index}`;
              ...
              // Add new highlight
              window.map.addSource(sourceId, { type: "geojson", data: geojson });
              window.map.addLayer({
                id: layerId,
                type: "line",
                source: sourceId,
              ...
              console.log(`🔴 Highlighted edge ${edgeIndex}: ${rec.reasoning || rec.action}`);
            }
          });

          // Store recommendations for later use
          window.currentAIRecommendations = recommendations;
        })();
